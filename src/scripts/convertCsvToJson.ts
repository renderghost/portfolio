...
function convertCaseStudies(data: RawCSVRow[]): CaseStudy[] {
	return data.map(row => ({
		...
		company: row.Company,
		relevantSkills:
			row['Relevant Skills']?.split(';').map((s: string) => s.trim()) ||
			[],
	}));
}
...
function convertJobs(data: RawCSVRow[]): Job[] {
	return data.map(row => ({
		...
		caseStudies:
			row['Case Studies']?.split(';').map((s: string) => s.trim()) || [],
		client: row.Client,
	}));
}
...
async function convertCsvToJson(): Promise<void> {
	...
	const companies = convertCompanies(
		parseCsvFile(path.join(inputDir, 'Barry Prendergast - Companies.csv')),
	);
	const skills = convertSkills(
		parseCsvFile(path.join(inputDir, 'Barry Prendergast - Skills.csv')),
	);
	const caseStudies = convertCaseStudies(
		parseCsvFile(path.join(inputDir, 'Barry Prendergast - Portfolio.csv')),
	);
	const jobs = convertJobs(
		parseCsvFile(path.join(inputDir, 'Barry Prendergast - Jobs.csv')),
	);
	const articles = convertArticles(
		parseCsvFile(path.join(inputDir, 'Barry Prendergast - Articles.csv')),
	);
